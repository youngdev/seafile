...
start (CcnetProcessor *processor, int argc, char **argv)
{
    ...
    g_string_append_printf(buf,
            "remote %s seafile-commit-upload-file-slave %s %s %s %s %s %s",
            processor->peer_id, task->repo_id, task->new_commit->commit_id,
            task->fs_sha1, task->topath, task->filename, task->session_token);
