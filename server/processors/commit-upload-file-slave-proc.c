...
start (CcnetProcessor *processor, int argc, char **argv)
{
    ...
    if (strlen(argv[0]) != 36 || strlen(argv[1]) != 40 || strlen(argv[2]) != 40) {
        ccnet_processor_send_response(processor, SC_BAD_ARGS, SS_BAD_ARGS, NULL, 0);
        ccnet_processor_done(processor, FALSE);
        return -1;
    }
    ...
    priv->repo_id[36] = '\0';
    memcpy(priv->new_commit, argv[1], 40);
    memcpy(priv->fs_sha1, argv[2], 40);
    priv->fs_sha1[40] = '\0';
    priv->topath = g_strdup(argv[3]);
    priv->filename = g_strdup(argv[4]);
    session_token = argv[5];
